...
3. Install deps + run verification commands from AGENTS.md: `cd {{worktreeDir}}` then follow the project's build/lint/typecheck instructions
...
5. If issues found → leave rejection feedback in TWO places, then send the task back to dev:
   ...
   - OpenClutch task comment (so agents see it on retry):
     `clutch tasks comment {{taskId}} --project {{projectSlug}} "REVIEW REJECTED (PR #{{prNumber}}): <feedback>" --author reviewer`

   After posting feedback, move the task back to `ready` so a dev agent re-picks it up with the feedback in context:
   `clutch tasks move {{taskId}} --project {{projectSlug}} ready`
...
3. **Review the diff** — `gh pr diff {{prNumber}}`
...
3. Use the CLI: `clutch tasks create --project {{projectSlug}} ...`
