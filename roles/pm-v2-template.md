...
```bash
clutch tasks update {{taskId}} --project {{projectSlug}} --description '<fleshed out markdown>' --role dev
```
...
```bash
clutch tasks signal {{taskId}} --project {{projectSlug}} --kind question --message "Specific question about ambiguity X. What is the expected behavior when Y happens?"
```
...
**Your job:** {{#if hasSignalResponses}}Analyze this ticket and break it down into actionable sub-tickets using the answers to your clarifying questions.{{else}}Triage this issue and either flesh it out and change role to `dev`, or create clarifying questions as blocking signals.{{/if}}
