...
export async function runWork(ctx: WorkContext): Promise<WorkPhaseResult> {
  ...
  let devCount = allActiveTasks.filter((t) => t.project_id === project.id && t.status === "in_progress" && (t.role === "dev" || !t.role)).length
  ...
  for (const task of sortedTasks) {
    ...
    try {
      const { sessionKey } = await agentManager.spawn({
        ...
        model,
        thinking: "off",
        timeoutSeconds: 3600,  // 1 hour — loop reaper handles staleness before this
